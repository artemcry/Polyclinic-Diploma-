...
<?
    ...
    if (isset($_POST["update"])) {
        ...
        $specialization = crypt_str::en(sql_valid($_POST["specialization"]));
        $education = crypt_str::en(sql_valid($_POST["education"]));
        ...
        $sq2 = "UPDATE `doctors` SET `specialization` = '{$specialization}', `education` = '{$education}' WHERE `user_id` = {$id};";
...
<div class="container">
  <div class="row justify-content-center">
    ...
    <h2 class="ms-3 mt-1">Мій профіль</h2>
    </div>
    <?=$msg?>
    <script>
    ...
    <?
        $sql = "SELECT * FROM `doctors` WHERE `user_id` = '{$_SESSION["user-id"]}'";
        $dctr = mysqli_fetch_all(mysqli_query($link, $sql))[0];
        $sql = "SELECT * FROM `users` WHERE `user_id` = '{$_SESSION["user-id"]}'";
        $usr = mysqli_fetch_all(mysqli_query($link, $sql))[0];
        $link->close();
    ?>
      ...
      <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input value="<?=crypt_str::de($dctr[1])?>" type="text" id="specialization" name="specialization"  class="form-control mb-2">
        </div>
        <div class="form-group">
          <label for="education">Освіта</label>
          <input value="<?=crypt_str::de($dctr[2])?>" type="text" id="education" name="education"  class="form-control mb-2">
        </div>
        <div class="form-group">
        ...
        <div class="form-group mt-1">
          <label for="profile-photo">Нове фото профілю (За необхідності)</label><br>
...
<script>
    function validateForm() {
        ...
        specialization === '' || education === '' || phone === '' || email === '') {
